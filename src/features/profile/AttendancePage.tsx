...
function periodDisplay(
  ...
  if (period) return t('profile.schedule.periodFallback', { n: period })
  return '—'
}

export function AttendancePage() {
  const { t, i18n } = useTranslation()
  const [lessons, setLessons] = useState<StudentLessonAbsencesPayload | null>(null)
  ...
  const lessonSummaryCards = useMemo(() => {
    const summary = lessons?.summary
    return {
      lessonRate: formatRate(summary?.attendance_rate),
      presentLessons: summary?.present_lessons ?? '—',
      absenceLessons: summary?.absence_lessons ?? '—',
      recordedLessons: summary?.recorded_lessons ?? '—',
    }
  }, [lessons])

  const maxLessonMonthAbsences = useMemo(() => {
    const values = lessons?.by_month?.map((item) => item.absence_lessons) ?? []
  ...
  const lessonAbsences = lessons?.absences ?? []
  const lessonByMonth = lessons?.by_month ?? []
  const academicYear = lessons?.filters?.academic_year
  ...
  return (
    <div className="w-full">
      ...
      <div className="mt-8">
        <div className="mb-4 flex items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-bold text-brand-dark">
              {t('profile.attendance.lessonsTitle')}
            </h2>
          </div>
          {lessonAbsences.length > 0 ? (
            <span className="rounded-lg bg-brand-primary/10 px-2.5 py-1 text-xs font-semibold text-brand-primary">
              {t('profile.attendance.lessonCount', { count: lessonAbsences.length })}
            </span>
          ) : null}
        </div>

        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
          {lessonSummaryMeta.map((item) => (
            <article
              key={item.key}
        ...
        {lessonAbsences.length === 0 ? (
          <div className="mt-4 rounded-2xl border border-dashed border-brand-dark/15 bg-muted/20 px-5 py-12 text-center text-sm text-brand-dark/45">
            {t('profile.attendance.emptyLessons')}
          </div>
        ) : (
          <ul className="mt-4 space-y-3">
            {lessonAbsences.map((row, index) => (
              <li
                key={`${row.date}-${row.period ?? index}-${row.subject?.id ?? index}`}
                    ...
                    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                      <div>
                        <p className="text-[11px] font-medium tracking-wide text-brand-dark/45 uppercase">
                          {t('profile.attendance.columns.subject')}
                        </p>
                      ...
                      <div>
                        <p className="text-[11px] font-medium tracking-wide text-brand-dark/45 uppercase">
                          {t('profile.attendance.columns.period')}
                        </p>
                        <p className="mt-1 text-sm font-medium text-brand-dark">
                          {periodDisplay(row.period, row.period_label, t)}
                        </p>
                      </div>
                    </div>
                    {row.recorded_by ? (
        ...
        <div className="mt-6">
          <div className="mb-4 flex items-center gap-2">
            ...
            <h3 className="text-base font-bold text-brand-dark">
              {t('profile.attendance.lessonsByMonthTitle')}
            </h3>
          </div>

          {lessonByMonth.length === 0 ? (
            <div className="rounded-2xl border border-dashed border-brand-dark/15 bg-muted/20 px-5 py-12 text-center text-sm text-brand-dark/45">
              {t('profile.attendance.emptyLessonMonths')}
            </div>
          ) : (
            <ul className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              {lessonByMonth.map((item) => {
                const ratio =
                  item.absence_lessons === 0 ? 0 : item.absence_lessons / maxLessonMonthAbsences
                return (
                  <li
                    key={item.month}
                  ...
                  >
                    ...
                    <div className="mt-3 h-2.5 overflow-hidden rounded-full bg-brand-dark/8">
                      <div
                        ...
                        style={{
                          width: `${Math.max(ratio * 100, item.absence_lessons > 0 ? 12 : 0)}%`,
                        }}
                      />
                    </div>
                  </li>
